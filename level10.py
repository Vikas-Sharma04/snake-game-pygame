...
class LEVEL10_MGR:
    ...
    def create_walls(self):
        walls = []
        for x in range(3, 7):
            walls.append(Vector2(x, 3)) 
            walls.append(Vector2(self.cell_number - x - 1, 3)) 
            walls.append(Vector2(x, self.cell_number - 3))
            walls.append(Vector2(self.cell_number - x - 1, self.cell_number - 3)) 
        for x in range(2, 18):
            walls.append(Vector2(x, 10)) 
        for y in range(3, 7):
            walls.append(Vector2(2, y))
            walls.append(Vector2(2, self.cell_number - y))
            walls.append(Vector2(self.cell_number - 3, y))
            walls.append(Vector2(self.cell_number - 3, self.cell_number - y))
        return walls
    ...
    def update(self):
        # Stop movement if game over or if the player has won
        if not self.paused and not self.game_over_flag and not self.win_flag:
    ...
    def check_collision(self):
        if self.fruit.pos == self.snake.body[0]:
            ...
            self.fruit_count += 1
            # Trigger Win Logic at 100
            if self.fruit_count >= 100:
    ...
    def draw_elements(self):
        ...
        # Draw Win Message
        if self.win_flag:
            self.draw_mid_text("YOU WIN! Level 10 Complete!")
    ...
    def draw_mid_text(self, text):
        ...
        # Message box for visibility
        bg_rect = rect.inflate(20, 20)
        pygame.draw.rect(self.screen, (106, 13, 173), bg_rect) # Purple background
...
async def main(current_score=0, speed=150):
    ...
    mgr = LEVEL10_MGR(screen, cs, cn, apple_img, font, SCREEN_UPDATE, current_score, speed)

    while True:
        # Check if the win timer has passed 3 seconds
        if mgr.win_flag and pygame.time.get_ticks() - mgr.win_timer > 3000:
            import level1
            await level1.main(mgr.fruit_count, max(60, mgr.speed - 10))
            return
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if not mgr.game_over_flag and not mgr.win_flag:
