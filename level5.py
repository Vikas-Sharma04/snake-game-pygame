...
class LEVEL5_MGR:
    ...
    def create_walls(self):
        walls = []
        for x in range(0, self.cell_number - 5):
            walls.append(Vector2(x, 4))
            walls.append(Vector2(self.cell_number - x, self.cell_number - 4))
        for y in range(0, int(self.cell_number / 2) - 1):
            walls.append(Vector2(self.cell_number - y, 10))
            walls.append(Vector2(y, self.cell_number - 10))
        return walls
...
async def main(current_score=0, speed=150):
    ...
    mgr = LEVEL5_MGR(screen, cs, cn, apple_img, font, SCREEN_UPDATE, current_score, speed)

    while True:
        if mgr.level_up_flag:
            import level6
            await level6.main(mgr.fruit_count, max(60, mgr.speed - 10))
            return
