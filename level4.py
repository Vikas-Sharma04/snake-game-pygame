...
class LEVEL4_MGR:
    ...
    def create_walls(self):
        walls = []
        mid = int(self.cell_number/2)
        for x in range(0, mid - 1):
            walls.append(Vector2(5, self.cell_number - x))
            walls.append(Vector2(self.cell_number - 5, self.cell_number - x))
        for y in range(1, mid - 1):
            walls.append(Vector2(5, y))
            walls.append(Vector2(self.cell_number - 5, y))
        return walls
...
async def main(current_score=0, speed=150):
    ...
    mgr = LEVEL4_MGR(screen, cs, cn, apple_img, font, SCREEN_UPDATE, current_score, speed)

    while True:
        if mgr.level_up_flag:
            import level5
            await level5.main(mgr.fruit_count, max(60, mgr.speed - 10))
            return
