...
class MAIN:
    ...
    def create_walls(self):
        walls = []
        for x in range(0, int(cell_number/2)):
            walls.append(Vector2(x, 14))  # Top wall
            walls.append(Vector2(cell_number-x, cell_number - 14))  # Bottom wall
        for y in range(1, int(cell_number/2)+1):
            walls.append(Vector2(5, y))  # Left wall
            walls.append(Vector2(cell_number - 5, cell_number-y))  # Right wall
        return walls
    ...
    def level_up(self):
        ...
        import level9
        level9.main(current_score, self.speed)  # Pass the score and speed increment
        return
