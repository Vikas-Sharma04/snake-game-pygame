...
class LEVEL8_MGR:
    ...
    def create_walls(self):
        walls = []
        half = int(self.cell_number / 2)
        # Symmetrical corner wall layout
        for x in range(0, half):
            walls.append(Vector2(x, 14))
            walls.append(Vector2(self.cell_number - x, self.cell_number - 14))
        for y in range(1, half + 1):
            walls.append(Vector2(5, y))
            walls.append(Vector2(self.cell_number - 5, self.cell_number - y))
        return walls
...
async def main(current_score=0, speed=150):
    ...
    mgr = LEVEL8_MGR(screen, cs, cn, apple_img, font, SCREEN_UPDATE, current_score, speed)

    while True:
        if mgr.level_up_flag:
            import level9
            await level9.main(mgr.fruit_count, max(60, mgr.speed - 10))
            return
