...
class LEVEL9_MGR:
    ...
    def create_walls(self):
        ...
        # Double internal horizontal bars
        for x in range(2, 18):
            walls.append(Vector2(x, 7)) 
            walls.append(Vector2(x, 13)) 

        # Corner vertical segments
        for y in range(2, 5):
...
async def main(current_score=0, speed=150):
    ...
    mgr = LEVEL9_MGR(screen, cs, cn, apple_img, font, SCREEN_UPDATE, current_score, speed)

    while True:
        if mgr.level_up_flag:
            import level10
            await level10.main(mgr.fruit_count, max(60, mgr.speed - 10))
            return
