...
class LEVEL6_MGR:
    ...
    def create_walls(self):
        walls = []
        # Top and bottom horizontal wall lines
        for x in range(3, self.cell_number - 3):
            walls.append(Vector2(x, 3))
            walls.append(Vector2(x, self.cell_number - 3))
        # Side vertical wall chunks
        for y in range(3, 8):
...
async def main(current_score=0, speed=150):
    ...
    mgr = LEVEL6_MGR(screen, cs, cn, apple_img, font, SCREEN_UPDATE, current_score, speed)

    while True:
        if mgr.level_up_flag:
            import level7
            await level7.main(mgr.fruit_count, max(60, mgr.speed - 10))
            return
