...
class FRUIT:
    def __init__(self, walls):
        self.walls = walls  # Store walls for collision checks
        self.randomize()
    ...
    def randomize(self):
        while True:
            ...
            if self.pos not in self.walls:  # Ensure fruit doesn't collide with walls
                break
...
class MAIN:
    def __init__(self, current_score, speed):
        ...
        self.fruit = FRUIT(self.wall_rects)
        self.level = 2
        self.fruit_count = current_score  # Initialize with the score passed from level 1
        self.speed = speed  # Initialize with the speed passed from level 1
    ...
    def game_over(self):
        if not self.game_over_flag:  # Play sound only once
    ...
    def create_walls(self):
        walls = []
        for x in range(0, cell_number):
            walls.append(Vector2(x, 1))  # Top wall
            walls.append(Vector2(x, cell_number - 1))  # Bottom wall
        for y in range(2, cell_number - 1):
            walls.append(Vector2(0, y))  # Left wall
            walls.append(Vector2(cell_number - 1, y))  # Right wall
        return walls

    def draw_walls(self):
        for wall in self.wall_rects:
            wall_rect = pygame.Rect(int(wall.x * cell_size), int(wall.y * cell_size), cell_size, cell_size)
    ...
    def check_fail(self):
        ...
        if self.snake.body[0] in self.wall_rects:
            self.game_over()
    ...
    def level_up(self):
        ...
        # Save current score and speed increment for the next level
        current_score = self.fruit_count
        self.speed = max(60, self.speed - 10)
        import level3  # Import the next level
        level3.main(current_score, self.speed)  # Pass the score and speed increment
        return
    ...
    def draw_banner(self):
        ...
        banner_text = f"Score: {self.fruit_count} | Level: {self.level}"
